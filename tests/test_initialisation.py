...
class TestInitialisation:
    ...
    def test_initialize_happy_path(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "src.initialisation.cfn_create_or_update", mock_cfn_create_or_update
        )
    ...
    def test_teardown_happy_path_empty_bucket(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "src.initialisation.get_management_bucket_name",
            get_mocked_management_bucket_name,
        )

        # Create mocked S3 bucket
        s3_conn = boto3.client("s3", region_name="us-east-1")
        s3_conn.create_bucket(Bucket=get_mocked_management_bucket_name())
        ...
        assert (
            s3_conn.list_objects_v2(Bucket=get_mocked_management_bucket_name())[
                "KeyCount"
            ]
            == 0
        )
