...
class TestStack:
    ...
    def test_load_stack_config_from_file_happy_path(self):
        cfg = Stack.load_stack_config_from_file(Path(__file__).parent.joinpath("test_stack_configs/config.json"))
        ...
        assert cfg.template_version.get_version_string() == Version("0.1").get_version_string()

    def test_load_stack_config_from_file_missing_some_attributes_errors(self):
        with pytest.raises(Exception):
            Stack.load_stack_config_from_file(Path(__file__).parent.joinpath("test_stack_configs/config_incomplete.json"))

    def test_load_stack_config_from_missing_file(self):
        with pytest.raises(Exception):
            Stack.load_stack_config_from_file(Path(__file__).parent.joinpath("test_stack_configs/MISSING!_config.json"))
    ...
    def test_load_stack_config_from_file_with_bad_values(self):
        for file_name in [
            ...
            with pytest.raises(Exception):
                print(file_name)
                Stack.load_stack_config_from_file(Path(__file__).parent.joinpath(f"test_stack_configs/{file_name}.json"))
    ...
    def test_deploy_happy_path(self, monkeypatch):
        ...
        monkeypatch.setattr("src.stack.get_management_bucket_url", get_mocked_management_bucket_url)
